...
function testHistoryWithReverts() {
    ...
    // so: "F D U" after planning reverse is: "U' U R ..." but after processing U1 we should get: "F D" and planned: "U R ..." (the pair U U' disappears)
    cube.revertOneMove();
    assertEquals(Movement.toText(cube.history), "F D U", "History should reflect processed moves");
    assertEquals(Movement.toText(cube.planned), "U\' U R L D B R F U", "Planned moves contain only moves that have not been processed yet");
    ...
    cube.revertOneMove();
    assertEquals(Movement.toText(cube.history), "F D U R L", "History should reflect processed moves");
    assertEquals(Movement.toText(cube.planned), "L\' L D B R F U", "Planned moves contain only moves that have not been processed yet");
    ...
    cube.revertOneMove();
    assertEquals(Movement.toText(cube.history), "F D U R", "History should reflect processed moves");
    assertEquals(Movement.toText(cube.planned), "R\' R L D B R F U", "Planned moves contain only moves that have not been processed yet");
    applyOnePlannedMove(cube); // Apply reverse move - delete entry from history
